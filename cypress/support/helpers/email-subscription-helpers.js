...
import {getLastUserEmail, getLastUserEmailUsingSentAt} from "./email-helpers";
...
export const Frequency = {
    ...
    Weekly: "Once a week",
    Instantly:
        "Each time we add or update a page (you may get more than one email a day)",
};
...
export const assertVerificationEmailIsSentAndVerifyEmail = (
    email,
    verificationMessage = "You've subscribed to emails about UKMCAB search results"
) => {
...
export const assertSearchUpdateSubscriptionEmailIsSent = (
    email,
    searchTerm
) => {
    getLastUserEmail(email).then((_email) => {
...
export const assertRequestToUnarchiveEmailIsSent = (email, cabName) => {
    getLastUserEmail(email).then((_email) => {
        expect(_email.isRequestToUnarchiveConfirmationEmail(cabName)).to.be.true;
    });
};

export const assertApproveRequestToUnarchiveAndPublishEmailIsSent = (
    email,
    cabName
) => {
    getLastUserEmail(email).then((_email) => {
        expect(
            _email.isApproveRequestToUnarchiveAndPublishConfirmationEmail(cabName)
        ).to.be.true;
    });
};
...
export const assertApproveRequestToUnarchiveAndSaveAsDraftEmailIsSent = (
    ...
    getLastUserEmail(email).then((_email) => {
        expect(
            _email.isApproveRequestToUnarchiveAndSaveAsDraftConfirmationEmail(cabName)
        ).to.be.true;
    });
};

export const assertDeclineRequestToUnarchiveAndPublishEmailIsSent = (
    email,
    cabName
) => {
    getLastUserEmail(email).then((_email) => {
...
export const assertDeclineRequestToPublishEmailIsSent = (
    ...
    getLastUserEmailUsingSentAt(email).then((_email) => {
        expect(
...
export const assertDeclineRequestToUnarchiveAndSaveAsDraftEmailIsSent = (
    email,
    cabName
) => {
    getLastUserEmail(email).then((_email) => {
        expect(
            _email.isDeclineRequestToUnarchiveAndSaveAsDraftConfirmationEmail(cabName)
        ).to.be.true;
    });
};
