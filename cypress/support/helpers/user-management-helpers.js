...
export const userAdminPath = (user) => {
    return `/user-admin/${encodeURIComponent(user.id)}`
}
export const userApprovePath = (userrequest) => {
    return `/user-admin/review-account-request/${userrequest.id}`
}

export const reviewRequestPath = (request) => {
    return `/user-admin/review-account-request/${encodeURIComponent(request.id)}`
}
...
export const getAccountRequests = () => {
    ...
    return cy.task('executeQuery', {
        db: 'main',
        container: 'user-account-requests',
        querySpec: querySpec
    }).then(results => {
