...
describe('UKMCAB RSS Feed', function () {
    ...
    const validateEntry = (entry, index, results) => {
        ...
        expect(entry.updated).to.match(isoDateRegex);
                ...
                if (feed.entry) {
                    if (Array.isArray(feed.entry)) {
                        feed.entry.forEach((entry, index) => {
                            validateEntry(entry, index, results);
                        });
                    } else {
                        validateEntry(feed.entry, 0, results);
                    }
                } else {
                    console.log('feed.entry is not defined');
                }
            })
        })
    })
    ...
    it('works for Cab page', function () {
        CabHelpers.getTestCab().then(cab => {
            ...
            cy.request({
                ...
                expect(feed.updated).to.match(isoDateRegex);
                expect(feed.author.name).to.eq('HM Government')
                ...
                expect(feed.entry.updated).to.match(isoDateRegex);
